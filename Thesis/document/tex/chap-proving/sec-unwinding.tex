...
\[
\begin{split}
{UnwindCall}(\ell,\ell') \equiv (\ell,\ell')\in\hat{E} \wedge \cmd{cmd}^\fun{f}(\ell,\ell')=(\overline{\texttt{x}}:=\mathtt{g}(\overline{p}))\wedge\method{idx}(\ell, \ell')=i  \\
...
\todo[inline]{The definition of \cmd{cmd} is not complete}
\[
\begin{array}{l}
