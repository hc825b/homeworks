...
program \prog{P} will not increase in size during unwinding $\fun{fb}$.
The reason is that $\fun{g}_k$ is introduced when $\fun{fa}$ is unwound.
...
\end{proposition}
Since $\fun{g}_k$ does not belongs to original program \prog{P},
it won't introduce recursion.
