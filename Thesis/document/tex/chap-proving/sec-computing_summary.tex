...
In order to help extracting summaries from the $K^\fun{f}$, $\method{unwind}(G^\fun{f})$ annotates in $K^\fun{f}$ the outermost pair of the entry and exit locations ${s_i}$ and ${e_i}$ of each unwound function $\fun{g}$ with an additional superscript $\fun{g}$, i.e., $s_i^\fun{g}$ and $e_i^\fun{g}$
The function $\method{mark}_\fun{f}(G^\fun{g})$ returns a CFG that is identical to $G^\fun{g}$, except that, for the case that no location with superscript $\fun{g}$ appears in $V$ (the locations of $G^\fun{f}$), it annotates the entry and exit locations, $s_k$ and $e_k$, of the returned CFG with superscript $\fun{g}$, i.e., $s_k^\fun{g}$ and $e_k^\fun{g}$. That is, 
\[
...
Function 
ComputeSummary($\prog{P}_k$, $\Pi (\underline{G}^{\fun{main}}_k, \TT)$)
...
\begin{doublespace}
  ...
  { 
    ...
    {
      \If{$I_{s_i^\fun{f}}$ contains return variables of $\mathtt{f}$}
      {
...
For each function $\fun{f}$ in the program $\prog{P}_k$, we first initialize its summary $S[\fun{f}]$ to $\TT$.
